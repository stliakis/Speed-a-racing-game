...
public class Tail extends Entity {
	...
	public static class TailItem implements Comparable{
		...
		public int compareTo(Object arg0) {
			if(((TailItem)arg0).addTime>addTime)return 1;
			else return -1;
		}
	}
	private int side;
	...
	public void create() {
		...
		addMechanism(new IntervalMechanism(this,0) {
			public void tick() {
				...
				if(left!=null){
					float z=parrent.pos.z-0.05f;
					if(side==1)z-=parrent.vel.x/10;
					else z+=parrent.vel.x/10;
					
					if(side==1){
						z+=parrent.vel.x/20;
					}else z-=parrent.vel.x/20;
					
					float alpha=Math.max(0.5f, 0.5f+(side==-1?parrent.vel.x:-parrent.vel.x))*parrent.vel.y;
					
					left.set(parrent.pos.x+side*0.08f-0.03f +parrent.vel.x/15, parrent.pos.y-0.2f,z,alpha);
				...
				if(right!=null){
					...
					right.set(parrent.pos.x+side*0.08f+0.03f +parrent.vel.x/15,parrent.pos.y-0.2f,z,alpha);
		...
		addMechanism(new IntervalMechanism(this,0) {
			...
			public void tick() {
				for(int c=0;c<tailItems.size();c++){
					...
					else tailItems.release(c);
				}
			}
		});
		
		addMechanism(new MeshMechanism(this,
				new Shape(new float[1000]),
				Shader.getShader("shaders/tail.vert", "shaders/tail.frag"),
