...
public class RainSystem extends Entity {
	public static class RainDrop {
		...
		public void set(float x,float y,float z,float vx,float vy,float vz){
			pos.set(x,y,z);
			vel.set(vx,vy,vz);
			color.set(0.2f, 0.2f,1, 1);
			subDrop=false;
		}
	}
	public static int TAG=gWorld.getNextTag();
	Pool<RainDrop> drops;
	public RainSystem(gWorld world) {
		super(world);
	}
	@Override
	public void create() {
		...
		initSpeed(5);
		drops=new Pool<RainDrop>(10000,RainDrop.class);
				...
				for(int c=0,indx=0;c<drops.size();c++){
					...
					if(drop.pos.z<0){
						...
						if(drop.subDrop==false){
							for(int i=0;i<3;i++){
								RainDrop subDrop=add(drop.pos.x,drop.pos.y,drop.pos.z+0.05f,Tools.randf(-0.2f, 0.2f),Tools.randf(-0.2f, 0.2f),0.4f);
								if(subDrop!=null)subDrop.subDrop=true;
							}
						}
					}
				}
		...
		addMechanism(new MeshMechanism(this,
				new Shape(dropsVerts),
				Shader.getShader("shaders/rainDrop.vert", "shaders/rainDrop.frag"),
				GL20.GL_LINES,
				 new VertexAttribute(Usage.Position, 3, "a_position"), new VertexAttribute(Usage.Color, 4, "a_color"))
				{
	...
	public RainDrop add(float x,float y,float z,float vx,float vy,float vz){
		RainDrop drop=drops.getFree();
