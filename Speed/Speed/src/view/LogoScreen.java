...
public class LogoScreen extends LoadingScreen {
	...
	Button btnText;
	public LogoScreen() {
		setCamera2D(new Vector(Gdx.graphics.getWidth(),
	...
	public void Update(Director director) {
		...
		if(timeon>0.5f && timeon<2){
			if(btnText.color.a<1)btnText.color.a+=0.05f*Director.delta;
			btnText.color.checkRange();
		} 
		
		if(timeon>2){
			if(btnText.color.a>0)btnText.color.a-=0.05f*Director.delta;
			else getDirector().setReadyToLoadAssets(true);
			btnText.color.checkRange();
